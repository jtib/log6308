...
def testImpactOfSVDDimensions(trainSet,testSet,items,dimensionsIndices):
    global fig
    lKNI = []
    lSVD = []
    lRandom = []
    model, users, locations, vec, dic = small_script.getDoc2Vec(trainSet, 1000)
    userDistances = testknn.userCosine(users)
    usersLocDistances = testknn.userLocCosine2(users, locations)
    dKNI = testknn.KNIapproach(usersLocDistances, 10)
    tags = metrics.getIndexFromTags(vec)
    locTags = metrics.getLocIndexFromTags(model.wv.index2word)
    for i in dimensionsIndices:
        print("computing SVD for {} dimensions...".format(i))
        dSVD = test_svd.svdPrediction(dic, items, i, 10)
        randomRes = testknn.getRandomRecommendation(2060, 2804, 10)
        lRandom.append(metrics.precisionAtK(randomRes, testSet,tags,locTags))
        lSVD.append(metrics.precisionAtKSVD(dSVD,testSet))
        lKNI.append(metrics.precisionAtK(dKNI[0], testSet, tags, locTags))

    plt.figure(fig)
    fig += 1
    plt.xscale('log')
    plt.plot(dimensionsIndices,lKNI,label='KNI')
    plt.plot(dimensionsIndices, lSVD, label='SVD')
    plt.plot(dimensionsIndices, lRandom, label='Random Baseline')
    plt.xlabel("nb of dimensions for SVD decomposition")
    plt.ylabel("precision at k")
    plt.legend()
    plt.draw()
    pass
...
def main():
    ...
    small_script.test_gensim()
    #démonstration de word2vec avec des données jouets, dans un espace à 2 dimensions.
    ...
    dimIndices = [1,3,5,10,20,50,100]
    testImpactOfSVDDimensions(trainSet,testSet,items,dimIndices)

    plt.show()

#Attention : il sepeut que le code se mette en pause quand une figure s'ouvre; il suffit alors de fermer la figure pour que les calculs reprennent.
#Le code n'est pas optimisé et prend donc plusieurs minutes à s'exécuter entièrement
if __name__=='__main__':
    print("Attention : il sepeut que le code se mette en pause quand une figure s'ouvre; \n il suffit alors de fermer la figure pour que les calculs reprennent. ")
    print("Le code n'étant pas optimisé, il met plusieurs minutes à s'exécuter (plusieurs SVD notamment)")
    main()
